...
export class BackgroundColorSettings implements AfterViewInit {
  ...
  protected bgColor : string = "#FFFFFFFF";
  private scrollLeft = 0;

  constructor(private noteManager : NoteManager) {
    effect(() => {
      this.bgColor = this.noteManager.GetNoteBackgroundColor();
  ...
  HighlightSelected() {
    if (this.scroller) {
      ...
      for (let i = 0; i < cards.length; i++) {
        if (cards[i].getAttribute('color') === this.bgColor) {
  ...
  UpdateSelection(event : MouseEvent) {
    ...
    if (selected !== target) {
      ...
      this.noteManager.UpdateBackgroundColor(color);
    }
  }
